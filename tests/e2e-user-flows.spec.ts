...
test.describe('End-to-End User Flows', () => {
  ...
  test('power user interaction flow', async ({ page }) => {
    ...
    const counterBtnColor = await counterBtn.evaluate(el => 
      getComputedStyle(el).backgroundColor
    );
    expect(counterBtnColor).toBe('rgb(231, 76, 60)');
  ...
  test('accessibility-focused user flow', async ({ page }) => {
    ...
    await expect(page.locator('#messageDisplay')).toContainText('Keyboard navigation test');
  });
  ...
  test('mobile user experience flow', async ({ page }) => {
    ...
    await expect(page.locator('#messageDisplay')).toContainText('Mobile test message');
  ...
  test('error recovery user flow', async ({ page }) => {
    ...
    await expect(page.locator('#messageDisplay')).toContainText('Please enter a message first!');
    ...
    await expect(page.locator('#messageDisplay')).toContainText('Now I entered a message');
    ...
    await expect(page.locator('#messageDisplay')).toContainText('Final valid message');
  });
  ...
  test('session continuity flow', async ({ page }) => {
    ...
    // Verify final state
    await expect(page.locator('#counter')).toHaveText('1');
    await expect(page.locator('#messageDisplay')).toContainText('Session test');
