...
test.describe('Interactive Elements Tests', () => {
  ...
  test('should increment click counter', async ({ page }) => {
    ...
    // Test color change after 5 clicks
    const buttonColor = await counterBtn.evaluate(el => getComputedStyle(el).backgroundColor);
    ...
    const redButtonColor = await counterBtn.evaluate(el => getComputedStyle(el).backgroundColor);
  ...
  test('should display user message', async ({ page }) => {
    ...
    await expect(messageDisplay).toContainText(`Your message: "${testMessage}"`);
    
    // Verify input is cleared after showing message
    await expect(messageInput).toHaveValue('');
    
    // Verify success styling
    const backgroundColor = await messageDisplay.evaluate(el => getComputedStyle(el).backgroundColor);
