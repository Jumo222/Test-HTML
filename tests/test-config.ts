...
export class TestUtils {
  static async waitForAnimation(
    page: any,
    timeout: number = TEST_CONFIG.animationTimeout
  ) {
  ...
  static async performMultipleClicks(
    page: any,
    selector: string,
    count: number,
    delay: number = 50
  ...
  static async takeElementScreenshot(
    ...
    await page
      .locator(selector)
      .screenshot({ path: `test-results/${name}.png` });
  }
  ...
  static async mockDate(page: any, date: string = "2025-01-15T12:00:00.000Z") {
    await page.addInitScript((mockDate: string) => {
      ...
      // @ts-ignore - Date mocking requires overriding global Date constructor
      Date = class extends OriginalDate {
        constructor(...args: any[]) {
          if (args.length === 0) {
            super(fixedDate.getTime());
          } else {
            // @ts-ignore - spreading args into Date constructor
...
export class HomePage {
  ...
  async getBackgroundColor(): Promise<string> {
    return await TestUtils.getComputedStyle(
      this.page,
      "body",
      "background-color"
    );
