...
test.describe('About Page Tests', () => {
  ...
  test('should display current time on demo button click', async ({ page }) => {
  ...
  test('should display correct page content', async ({ page }) => {
    ...
    // Verify HTML section
    const htmlSection = techItems.filter({ hasText: 'HTML' });
    await expect(htmlSection).toContainText('HyperText Markup Language');
    await expect(htmlSection).toContainText('Semantic elements organize content meaningfully');
    
    // Verify CSS section
    const cssSection = techItems.filter({ hasText: 'CSS' });
    await expect(cssSection).toContainText('Cascading Style Sheets');
    await expect(cssSection).toContainText('Responsive design adapts to different screen sizes');
    
    // Verify JavaScript section
    const jsSection = techItems.filter({ hasText: 'JavaScript' });
    await expect(jsSection).toContainText('adds interactivity and dynamic behavior');
  ...
  test('should display code example', async ({ page }) => {
    ...
    await expect(codeExample).toContainText("document.getElementById('demoBtn')");
  ...
  test('should have working navigation from about page', async ({ page }) => {
  ...
  test('should load JavaScript file correctly', async ({ page }) => {
    ...
    expect(consoleLogs.some(log => log.includes('Welcome to your first web application!'))).toBe(true);
    expect(consoleLogs.some(log => log.includes('Open the browser developer tools'))).toBe(true);
  });

  test('should verify time display formatting', async ({ page }) => {
    const demoBtn = page.locator('#demoBtn');
    const timeDisplay = page.locator('#timeDisplay');
    ...
    // Verify proper formatting (should contain date and time labels)
    expect(timeContent).toMatch(/Current Date:.*\d+/);
    expect(timeContent).toMatch(/Current Time:.*\d+/);
    
    // Verify styling is applied
    const backgroundColor = await timeDisplay.locator('div').first().evaluate(el => 
      getComputedStyle(el).backgroundColor
    );
    expect(backgroundColor).toBe('rgb(36, 183, 151)');
  });

  test('should verify demo section structure', async ({ page }) => {
    const demoSection = page.locator('.demo-section');
    
    await expect(demoSection).toBeVisible();
    await expect(demoSection.locator('h2')).toHaveText('Interactive Demo');
    await expect(demoSection.locator('p')).toContainText('Click the button below to see JavaScript in action');
